...
def test_initDuckDB():
    spatial = duckdb.sql("SELECT extension_name, installed, description FROM duckdb_extensions() WHERE extension_name = 'spatial';")
    rowSpatial = spatial.fetchone()
    postgres = duckdb.sql("SELECT extension_name, installed, description FROM duckdb_extensions() WHERE extension_name = 'postgres_scanner';")
    rowPostgres = postgres.fetchone()
    assert(rowSpatial[1] == True)
    assert(rowPostgres[1] == True)
