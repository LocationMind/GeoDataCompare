...
# Length of the user interface

ui.page_opts(full_width=True, fillable=True)
...
quality_criterias = {
    ...
    "strong_comp":"Strong connected components",
...
ICONS = {
    ...
    "strong_comp":fa.icon_svg("arrow-right"),
...
with ui.sidebar(open="desktop", bg="#f8f8f8", width=350):
    ...
    with ui.accordion(id="acc", open=True):
        ...
        with ui.accordion_panel("Style components", class_= "background-grey"):
            ...
            ui.input_action_button("update_dataframe", "Update style", class_ = "mt-3 btn btn-outline-info")
...
def getCriterionInformation():
    ...
    if input.select_criterion() == "conn_comp" or input.select_criterion() == "strong_comp":
...
@reactive.effect
def colorRange():
    if currentCriterion() in ["conn_comp", "strong_comp"]:
        # Get map layers
        osmLayers = reactive_read(osm_map.widget, "layers")
        omfLayers = reactive_read(omf_map.widget, "layers")
