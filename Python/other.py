...
# Read data
path_data = os.path.join("..", "Data", "test", "segment", "*.parquet")
...
# Drop table
duckdb.execute("DROP TABLE IF EXISTS overturemap.public.road_tokyo CASCADE;")

duckdb.execute(f"""CREATE TABLE overturemap.public.road_tokyo AS (SELECT
               id,
...
duckdb.execute("CREATE INDEX road_tokyo_id_idx ON overturemap.public.road_tokyo (id);")

end = time.time()

print(f"Index : {end - start} seconds")

rel = duckdb.sql("SELECT count(*) FROM overturemap.public.road_tokyo;")
rel.show()
...
duckdb.execute("CALL postgres_execute('overturemap', 'ALTER TABLE IF EXISTS public.road_tokyo ADD COLUMN geom geometry;')")
duckdb.execute("CALL postgres_execute('overturemap', 'ALTER TABLE IF EXISTS public.road_tokyo ADD COLUMN theme character varying;')")
duckdb.execute("CALL postgres_execute('overturemap', 'ALTER TABLE IF EXISTS public.road_tokyo ADD COLUMN type character varying;')")

duckdb.execute("""CALL postgres_execute('overturemap', 'UPDATE public.road_tokyo 
               SET geom = public.ST_GeomFromText(geom_wkt, 4326), theme = ''transportation'', type = ''road'';')""")


print(f"Alter + Update : {end - start} seconds")

duckdb.execute("""CALL postgres_execute('overturemap', 'CREATE INDEX road_tokyo_geom_idx
               ON public.road_tokyo
               USING GIST (geom);')""")
