...
def getColorRange(number: int, data: list) -> list[int, int, int]:
...
def getAreasCRS(gdf: gpd.GeoDataFrame, columnName: str = "name") -> dict[str, int]:
    ...
    gdfCopy["crs"] = gdfCopy["centroid"].apply(
        getUTMProj, lambda point: utm.from_latlon(point.y, point.x)
    )
...
def setMapState(view_state: dict, map: lon.Map):
...
def getCriterion(
    criteria_classes: dict[str, c.Criterion], criterion: str
) -> c.Criterion:
...
generalValues = reactive.value(
    DefaultGeneralValues(datasetA=datasetA, datasetB=datasetB)
)

# Variable for the map state
formerMapState = reactive.value(
    lon.models.ViewState(latitude=36.390, longitude=138.812, zoom=7, pitch=0, bearing=0)
)
...
def getData():
    ...
    if currentAreaValue != area:
        ...
        # Get general values
        generalValues.set(
            GeneralValues(area, engine, datasetA=datasetA, datasetB=datasetB)
        )
    ...
    if (not isinstance(currentCriterionValue, criterionClass)) or (
        currentAreaValue != area
    ):
        # Create criterion instance
        currentCriterion.set(
            criterionClass(
    ...
    if not isinstance(currentThemeValue, criterionTheme) or (currentAreaValue != area):
        currentTheme.set(
            criterionTheme(area, crs, engine, datasetA=datasetA, datasetB=datasetB)
        )
...
with ui.sidebar(open="desktop", bg="#f8f8f8", width=350):
    ...
    with ui.accordion(id="acc", open=True):
        ...
        with ui.accordion_panel("Common styles", class_="background-sidebar"):
            ...
            ui.input_numeric(
                "radius_min_pixels", "Radius min pixel (point)", 2, min=1, max=10
            )

            ui.input_numeric(
                "width_min_pixels", "Width min pixel (line)", 1, min=1, max=10
            )

            ui.input_numeric(
                "line_min_pixel", "Line min pixel (polygon)", 0, min=0, max=10
            )
            ...
            def colorPickerPoint() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_point = ipywidgets.ColorPicker(
                    concise=True, description="Point color", value="#0000FF"
                )
            ...
            def colorPickerLine() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_line = ipywidgets.ColorPicker(
                    concise=True, description="Line color", value="#FF0000"
                )
            ...
            def colorPickerPolygonFill() -> ipywidgets.ColorPicker:
                ...
                Returns:
                    ipywidgets.ColorPicker: color picker for lines style.
                """
                color_picker_polygon_fill = ipywidgets.ColorPicker(
                    concise=True, description="Polygon color (fill)", value="#00FF00"
                )
            ...
            def colorPickerPolygonLine() -> ipywidgets.ColorPicker:
                ...
                Returns:
                    ipywidgets.ColorPicker: color picker for lines style.
                """
                color_picker_polygon_line = ipywidgets.ColorPicker(
                    concise=True, description="Polygon color (line)", value="#000000"
                )
        ...
        with ui.accordion_panel("Style components", class_="background-sidebar"):
            ...
            def legendComponents() -> render.DataGrid:
                ...
                return render.DataGrid(dataFrame, styles=stylesDF, editable=True)
        ...
        with ui.accordion_panel("Style isolated nodes", class_="background-sidebar"):
            ...
            def colorPickerIsolated() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_isolated = ipywidgets.ColorPicker(
                    concise=True, description="Isolated nodes color", value="#FF0000"
                )
            ...
            def colorPickerNotIsolated() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_not_isolated = ipywidgets.ColorPicker(
                    concise=True,
                    description="Non isolated nodes color",
                    value="#00FF00",
                )
        ...
        with ui.accordion_panel("Style overlap indicator", class_="background-sidebar"):
            ...
            def colorPickerOverlap() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_overlap = ipywidgets.ColorPicker(
                    concise=True, description="Overlap color", value="#00FF00"
                )
            ...
            def colorPickerNotOverlap() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_not_overlap = ipywidgets.ColorPicker(
                    concise=True, description="Non overlap color", value="#FF0000"
                )
                return color_picker_not_overlap

        # Style corresponding nodes layer
        with ui.accordion_panel(
            "Style corresponding nodes", class_="background-sidebar"
        ):
            ...
            def colorPickerCorresponding() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_corresponding = ipywidgets.ColorPicker(
                    concise=True, description="Corresponding color", value="#00FF00"
                )
            ...
            def colorPickerNotCorresponding() -> ipywidgets.ColorPicker:
                ...
                """
                color_picker_not_corresponding = ipywidgets.ColorPicker(
                    concise=True, description="Non corresponding color", value="#FF0000"
                )
            ...
            with ui.card_header():
                ...
                def getCardHeaderCriterion() -> str:
                    ...
                    return (
                        ...
                        f" {currentCriterion().displayNameCriterion}",
                    )
    ...
    with ui.layout_columns(col_widths=[6, 6, 6, 6], row_heights=[2, 1]):
        ...
        with ui.card(full_screen=True):
            ...
            def datasetA_map() -> lon._map.Map:
                ...
                layers = [
                    currentCriterion().datasetALayer
                ] + currentCriterion().datasetAOtherLayers
        ...
        with ui.card(full_screen=True):
            ...
            def datasetB_map() -> lon._map.Map:
                ...
                # Layers to display on the map
                layers = [
                    currentCriterion().datasetBLayer
                ] + currentCriterion().datasetBOtherLayers
...
with ui.nav_control():
    ui.a(
        ui.img(src=logo, alt="LocationMind Inc. logo", style="width:32px;height:32px;"),
...
def updateStyleDataFrame():
    ...
    ui.remove_ui(selector="#style-dataframe style")

    # Import new style balise
    ui.insert_ui(ui.tags.style(style), "#style-dataframe", where="beforeend")
...
def updateLayers():
    ...
    if (
        ...
        and isinstance(polygonWidth, int)
    ):
...
def colorBoolean():
    ...
    # Verify criterion
    if isinstance(
        currentCriterion(), (c.OverlapIndicator, c.IsolatedNodes, c.CorrespondingNodes)
    ):
        ...
        for layer in datasetBLayers + datasetALayers:
            ...
            gdfcopy["color"] = gdfcopy[currentCriterion().columnCriterion].apply(
                lambda x: colorTrue if x else colorFalse
            )
...
def colorRange():
    ...
    if isinstance(
        currentCriterion(),
        (c.ConnectedComponents, c.StrongComponents, c.PlaceGridDensity),
    ):
        # Get map layers
        datasetALayers = reactive_read(datasetA_map.widget, "layers")
        datasetBLayers = reactive_read(datasetB_map.widget, "layers")
        ...
        for layer in datasetBLayers + datasetALayers:
            ...
            if isinstance(layer, lon._layer.ScatterplotLayer):
                ...
                gdfcopy["color"] = gdfcopy["cardinality"].apply(
                    getColorRange, args=(editData,)
                )
            ...
            if isinstance(layer, lon._layer.PathLayer):
                # Line color and width
                colorLine = getColorFromColorPicker(colorPickerLine)
            ...
            if isinstance(layer, lon._layer.PolygonLayer):
            ...
            if returnPatch["value"] >= 0:
                ...
                if aboveValue < returnPatch["value"] < underValue:
                    ...
                    returnPatches.append(
                        {"row_index": row, "column_index": col, "value": val}
                    )
