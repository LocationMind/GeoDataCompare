...
class Theme(ABC):
    ...
    @abstractmethod
    def calculateDataFrame(self, tableName: str, engine: sqlalchemy.Engine):
        pass
    ...
    def __init__(
        ...
        self.datasetAClasses = self.calculateDataFrame(
            self._datasetALayerTemplate.format(area.lower()), engine
        )
        self.datasetBClasses = self.calculateDataFrame(
...
class Graph(Theme):
    ...
    def calculateDataFrame(
        ...
        if gdf.empty:
            ...
            gdfSumup = (
                gdf[[self.columnGroupBy, "length"]]
                .groupby(self.columnGroupBy, dropna=False)["length"]
                .agg(["sum", "count"])
            )
...
class Building(Theme):
    ...
    def __init__(
        ...
        self.dfHeader = (
            f"Number of buildings and total area (in km2) per class in {area}"
        )
    ...
    def calculateDataFrame(
        ...
        if gdf.empty:
            ...
            # Agregate per class and calculate the sum and count for each class
            gdfSumup = (
                gdf[[self.columnGroupBy, "area"]]
                .groupby(self.columnGroupBy, dropna=False)["area"]
                .agg(["sum", "count"])
            )
...
class Place(Theme):
    ...
    def calculateDataFrame(
        ...
        if gdf.empty:
            ...
            gdfSumup = (
                gdf[[self.columnGroupBy]]
                .groupby(self.columnGroupBy, dropna=False)[self.columnGroupBy]
                .agg(["size"])
            )
